...
// Byte Clock Frequency Calculator
const txLineRateInput = document.getElementById('tx-line-rate');
const calcByteClockBtn = document.getElementById('calc-byteclock-btn');
const byteClockOutput = document.getElementById('byteclock-output');
const byteClockForm = document.getElementById('byteclock-form');

if (txLineRateInput && calcByteClockBtn && byteClockOutput && byteClockForm) {
  calcByteClockBtn.addEventListener('click', function () {
    const txLineRate = parseFloat(txLineRateInput.value);
    const gear = 8; // Fixed
    if (isNaN(txLineRate) || txLineRate <= 0) {
      byteClockOutput.textContent = 'Please enter a valid TX Line Rate.';
      return;
    }
    const byteClock = txLineRate / gear;
    byteClockOutput.textContent = byteClock + ' MHz';
  });
  byteClockForm.addEventListener('reset', function () {
    byteClockOutput.textContent = '';
  });
}